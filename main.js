...
function UsersCard(){
    ...
    if (newCard.innerHTML < 10) {
        ...
        const number = Math.floor(Math.random() * (3 - 0) + 0)
            if(number == 0){
                const img = document.createElement("img")
                img.setAttribute('src','./Playing Cards/PNG-cards-1.4/jack_of_clubs.png')
                userCardsDiv.append(img)
            }
            else if( number == 1){
                const img = document.createElement("img")
